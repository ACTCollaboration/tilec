...
parser.add_argument("oversion", type=str,help='Version name.')
...
opath = sints.dconfig['actsims']['fg_res_path'] + "/"+ args.oversion + "_" + args.region +  "/"
...
for task in my_tasks:
    ...
    if flmax<=flmin:  #!!!!!!
        ...
        try:
            if low_ell:
                ...
                popt,pcov = curve_fit(ffunc,ccents[sel],res[sel]*ccents[sel]**2.,p0=[1,1,1e2],bounds=([0,0,0],[np.inf,2,np.inf]),absolute_sigma=True,sigma=errs[sel]*ccents[sel]**2.)
                pfit = lambda x: ffunc(x,popt[0],popt[1],popt[2])/x**2
                print("a1:", popt[0], "e1:",popt[1], "w:" ,popt[2])
        except:
            pfit = lambda x: x*0
        ...
        pl.add_err(ccents,res,yerr=errs,marker="o",ls="none",zorder=0)
        pl.add(ccents[sel],pfit(ccents[sel]),ls='--')
        # pl._ax.set_xlim(flmin,flmax)
        pl._ax.set_ylim(0.1,1e8)
        pl.done("%sres_%s_%s_%s.png" % (opath,region,qid1,qid2),verbose=False)
        ...
        # pl._ax.set_xlim(flmin,flmax)
        pl._ax.set_ylim(-10,5e6)
        pl.hline(y=0)
        pl.done("%sreslin_%s_%s_%s.png" % (opath,region,qid1,qid2),verbose=False)


        scls = pfit(fells)
        #scls[fells<20] = 0
        scls[fells<gellmin] = 0


        # bqids = ['p08','p07','p06'] # !!!
        # if (qid1 in bqids) or (qid2 in bqids): # !!!
        #     scls[fells<1000] = 0
        ...
        pl = io.Plotter(xyscale='linlog',scalefn=lambda x: x**2./2./np.pi,xlabel='l',ylabel='D')
        pl.add_err(ccents,res,yerr=errs,marker="o",ls="none",zorder=0,alpha=0.5)
        pl.add(ncents,n1d)
        pl.add(ncents,n1d1)
        pl.add(ncents,n1d2)
        ...
        pl.done("%sresfull_%s_%s_%s.png" % (opath,region,qid1,qid2),verbose=False)


        io.save_cols("%sfgcov_%s_%s.txt" % (opath,qid1,qid2),(fells,scls))
    ...
    #     """
    #     Take autopower
    #     Compare to theory_cmb + theory_tsz + save_res + save_noise
    #     Should be exact
        
    #     """
    #     fres = maps.interp(fells,scls)(ccents)
    #     pred = (fres + stheory.get_theory_cls(f1,f2,a_cmb=1,a_gal=0,a_cibp=0,a_cibc=0,a_radps=0,a_ksz=0,a_tsz=1))*fbeam1(ccents)*fbeam2(ccents) + maps.interp(ncents,n1d)(ccents)
    #     pred0 = (stheory.get_theory_cls(f1,f2,a_cmb=1,a_gal=0,a_cibp=0,a_cibc=0,a_radps=0,a_ksz=0,a_tsz=1))*fbeam1(ccents)*fbeam2(ccents) + maps.interp(ncents,n1d)(ccents)

    #     dm = sints.PlanckHybrid(region=mask)
    #     array = sints.arrays(qid1,'freq')
    #     dsplits1 = dm.get_splits(arrays=[array],ncomp=1,srcfree=True)[0][:,0]
    #     wins1 = dm.get_splits_ivar(arrays=[array],ncomp=1)[0][:,0]
    #     array = sints.arrays(qid2,'freq')
    #     dsplits2 = dm.get_splits(arrays=[array],ncomp=1,srcfree=True)[0][:,0]
    #     wins2 = dm.get_splits_ivar(arrays=[array],ncomp=1)[0][:,0]
    #     ssplits1 = enmap.read_map("/scratch/r/rbond/msyriac/data/scratch/tilec/test_sim_galtest_00_0000_deep56/split_%s.fits" % qid1)[:,0,...]
    #     ssplits2 = enmap.read_map("/scratch/r/rbond/msyriac/data/scratch/tilec/test_sim_galtest_00_0000_deep56/split_%s.fits" % qid2)[:,0,...]

    #     pl = io.Plotter(xyscale='linlog',scalefn=lambda x: x**2./2./np.pi,xlabel='l',ylabel='D')
    #     pl.add(ccents,pred,lw=2,color='k')
    #     pl.add(ccents,pred0,lw=2,color='k',ls="--")
    #     for ind,lab in zip(range(2),['data','sim']):
    #         if ind==0:
    #             splits1 = dsplits1
    #             splits2 = dsplits2
    #         elif ind==1:
    #             splits1 = ssplits1
    #             splits2 = ssplits2
    #         coadd1 = (splits1*wins1).sum(axis=0)/wins1.sum(axis=0)
    #         coadd1[~np.isfinite(coadd1)] = 0
    #         coadd2 = (splits2*wins2).sum(axis=0)/wins2.sum(axis=0)
    #         coadd2[~np.isfinite(coadd2)] = 0
    #         kmap1 = enmap.fft(coadd1*mask,normalize='phys')
    #         kmap2 = enmap.fft(coadd2*mask,normalize='phys')
    #         w2 = np.mean(mask**2.)
    #         p2d = (kmap1*kmap2.conj()).real/w2
    #         #cbin_edges = np.arange(20,8000,20)
    #         cbin_edges = np.arange(20,8000,40)
    #         cbinner = stats.bin2D(modlmap,cbin_edges)
    #         ccents,p1d = cbinner.bin(p2d)

    #         pl.add(ccents,p1d,lw=2,label=lab)
    #     # pl._ax.set_xlim(10,6500)
    #     # pl._ax.set_ylim(10,1e8)
    #     pl._ax.set_xlim(10,500)
    #     pl._ax.set_ylim(1e2,1e4)
    #     pl.done("%srespred_%s_%s_%s.png" % (opath,region,qid1,qid2),verbose=True)
