...
"""
reproduce various ``effective frequencies'' for ACT MBAC bandpasses from https://phy-wiki.princeton.edu/actwiki/pmwiki.php?n=Bandpasses.Bandpasses?action=download&upname=sources_May062010.pdf
...
For AR1/2/3, use files with upper and lower truncations determined in my updated/corrected version of Rahul's notebook.
"""
TCMB = 2.726  # Kelvin
...
N_freqs = 3 #AR1/2/3
ACT_freqs_GHz = np.array([148.,218.,277.])
ACT_file_dir = '../data/'
ACT_files = ['MBAC_AR1_148GHz_trunc.txt','MBAC_AR2_218GHz_trunc.txt','MBAC_AR3_277GHz_trunc.txt']
al_1 = 0.0
...
for i in xrange(N_freqs):
    ...
    dnu = nu[1]-nu[0] #all of the ACT MBAC bandpass files have equally spaced samples in frequency
    fnu = ACT_loc_trans
    ...
    print('nu_eff_CMB =', "{0:.1f}".format(nu_eff_CMB/1.e9), '+/-', 3.5, 'GHz') #uncertainty is due to systematic uncertainty in FTS measurements
    ...
    print('nu_eff_SZ =', "{0:.1f}".format(nu_eff_SZ/1.e9), '+/-', 3.5, 'GHz')
